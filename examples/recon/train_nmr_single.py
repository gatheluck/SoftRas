...
MODEL_DIRECTORY = 'data/results/nmr'
...
parser.add_argument('--renderer_type', type=str, default='nmr')
...
# setup transform
if args.renderer_type == 'nmr':
    args.transform = sr.Transform(camera_mode='look_at', 
                                  P=None, 
                                  dist_coeffs=None, 
                                  orig_size=512,
                                  perspective=True, 
        ...
        if i % args.print_freq == 0:
            if args.renderer_type == 'softras':
                ...
                print('Iter: [{0}/{1}]\t'
                    'Time {batch_time.val:.3f}\t'
                    'Loss {loss.val:.3f}\t'
                    'lr {lr:.6f}\t'.format(i, args.num_iterations,
                                            batch_time=batch_time, loss=losses, 
                                            lr=lr))
